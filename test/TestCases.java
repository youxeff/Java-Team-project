...
public class TestCases {
     ...
     * Tests methods that do not involve direct user input or output.
     */
    @Nested
    class MainTest {
        ...
        /**
         * Sets up the test environment by initializing the Main instance and Marketplace.
         */
        @BeforeEach
        void setUp() {
            main = new Main();
        ...
        /**
         * Tests the constructor of the Main class.
         * Ensures that the Main instance is created successfully.
         */
        @Test
        void testMainConstructor() {
            assertNotNull(main, "Main instance should not be null");
        }

        /**
         * Tests the registerUser method.
         * Ensures that a new user is successfully registered in the marketplace.
         */
        @Test
        void testRegisterUser() throws IOException {
            MarketplaceUser newUser = new MarketplaceUser("John", "Doe", "jdoe", "password123");
            marketplace.updateUserData(newUser);

            MarketplaceUser loadedUser = MarketplaceUser.loadUser("jdoe");
            assertNotNull(loadedUser, "User should be successfully registered and loaded");
        ...
        void testLoginUser() throws IOException {
            MarketplaceUser newUser = new MarketplaceUser("Purdue", "Pete", "pp", "securePassword");
            marketplace.updateUserData(newUser);

            MarketplaceUser loadedUser = MarketplaceUser.loadUser("pp");
            assertNotNull(loadedUser);
            assertTrue(loadedUser.verifyPassword("securePassword"));
        }

        /**
         * Tests the updateBalance method.
         * Ensures that the user's balance is updated correctly.
         */
        @Test
        void testUpdateBalance() throws IOException {
            MarketplaceUser user = new MarketplaceUser("Fake", "Name", "fName", "password456");
            user.setBalance(100.0);
            marketplace.updateUserData(user);

            user.setBalance(user.getBalance() + 50.0); // Add funds
            assertEquals(150.0, user.getBalance());
        }

        /**
         * Tests the displayUserProfile method.
         * Ensures that the user's profile information is displayed correctly.
         */
        @Test
        void testDisplayUserProfile() throws IOException {
            MarketplaceUser user = new MarketplaceUser("Will", "Bosi", "wBosi", "password789");
            user.setBalance(200.0);
            marketplace.updateUserData(user);

            assertEquals("Will", user.getFirstName());
            assertEquals("Bosi", user.getLastName());
            assertEquals("wBosi", user.getUserName());
            assertEquals(200.0, user.getBalance());
        }

        /**
         * Tests the sellItem method.
         * Ensures that an item is successfully added to the marketplace.
         */
        @Test
        void testSellItem() throws IOException {
            MarketplaceUser seller = new MarketplaceUser("Kevin", "Durant", "kdtrey", "password123");
            marketplace.updateUserData(seller);

            Apparel apparel = new Apparel("T-Shirt", 20.0, seller, "tshirt.png", "Clothing", "M", "Red", "Nike");
            marketplace.addItem(apparel);

            List<Item> items = marketplace.searchByCategory("Clothing");
            assertEquals(1, items.size());
            assertEquals("T-Shirt", items.get(0).getName());
        }

        /**
         * Tests the buyItem method.
         * Ensures that an item is successfully purchased from the marketplace.
         */
        @Test
        void testBuyItem() throws IOException {
            MarketplaceUser seller = new MarketplaceUser("Steph", "Curry", "scurry", "password123");
            seller.setBalance(100.0);
            marketplace.updateUserData(seller);

            MarketplaceUser buyer = new MarketplaceUser("Lebron", "James", "lbj", "password456");
            buyer.setBalance(200.0);
            marketplace.updateUserData(buyer);

            Vehicle vehicle = new Vehicle("Car", 150.0, seller, "car.png", "Vehicle", 10000, 2020, "Toyota");
            marketplace.addItem(vehicle);

            boolean purchaseResult = marketplace.purchaseItem(vehicle, buyer);
            assertTrue(purchaseResult);
            assertEquals(50.0, buyer.getBalance());
            assertEquals(250.0, seller.getBalance());
        }
    }
}